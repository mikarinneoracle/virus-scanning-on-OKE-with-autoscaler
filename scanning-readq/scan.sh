...
if [ $isInFile -eq 0 ]; then
   ...
   /root/bin/oci os object put --bucket-name scanning-ms-alert-report --region eu-amsterdam-1 --file report.txt --force --auth instance_principal
else
   echo "################# Scanning found no infected files #################"
   /root/bin/oci os object put --bucket-name scanned-ms --region eu-amsterdam-1 --file report.txt --force --auth instance_principal
